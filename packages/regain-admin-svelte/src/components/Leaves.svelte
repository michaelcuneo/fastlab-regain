  ...
  const addLeaves = () => {
    ...
    for(let i = 0; i < 20; i++) {
      leaf.push({
        ...
        top: Math.floor(Math.random() * (growthFrame.offsetWidth - -rect.x) + -rect.x),
        left: Math.floor(Math.random() * (growthFrame.offsetWidth - -rect.x) + -rect.x),
...
{:then leaf}
  {#each leaf as leave, i}
    ...
    >
      ...
      <path class="leaf" d="M34.31,14.12c4.09,12.32,7.3,34.16-6.13,31.55S26.82,23.8,34.31,14.12Z"/>
    </svg>
  {/each}
{/await}
