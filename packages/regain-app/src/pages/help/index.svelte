<script lang="ts">
  ...
  import { resetState } from "../../components/resetState.js";
  ...
  onMount(() => {
    ...
    editMessageSubscription = (
      API.graphql(
        graphqlOperation(onUpdateMessages)
      ) as GraphQLSubscription<any>
    ).subscribe({
      next: ({ value }) => updateReceived(value),
      error: (error: any) => console.log(error),
    ...
    removeMessageSubscription = (
      API.graphql(
        graphqlOperation(onDeleteMessages)
      ) as GraphQLSubscription<any>
    ).subscribe({
...
<LayoutGrid>
  <Cell span={4}>
    <Card>
      <Paper>
        <Title
          >The following button will reset the cache and clean all app functions
          to their default, this button will also log you out.</Title
        >
    ...
    <Card>
      <Content>
        ...
        <message-entry>
          ...
          <Button
            variant="raised"
            on:click={() => newMessage !== null && submitMessage()}>send</Button
          >
        </message-entry>
      </Content>
    </Card>
