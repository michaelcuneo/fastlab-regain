<script lang="ts">
  ...
  onMount(() => {
    usersettings.value.groups.items.map((group: any) => {
      if (group.groupID === activeAreas.value[0].id) activeAreas.value[0].status = true;
      if (group.groupID === activeAreas.value[1].id) activeAreas.value[1].status = true;
      if (group.groupID === activeAreas.value[2].id) activeAreas.value[2].status = true;
      if (group.groupID === activeAreas.value[3].id) activeAreas.value[3].status = true;
      if (group.groupID === activeAreas.value[4].id) activeAreas.value[4].status = true;
      if (group.groupID === activeAreas.value[5].id) activeAreas.value[5].status = true;
      if (group.groupID === activeAreas.value[6].id) activeAreas.value[6].status = true;
    });
  });
  ...
  const activateArea = async (area: string) => {
    switch (area) {
      case "head":
        activeAreas.value[0].status = !activeAreas.value[0].status;
        break;
      case "shoulder":
        activeAreas.value[1].status = !activeAreas.value[1].status;
        break;
      case "arm":
        activeAreas.value[2].status = !activeAreas.value[2].status;
        break;
      case "chest":
        activeAreas.value[3].status = !activeAreas.value[3].status;
        break;
      case "stomach":
        activeAreas.value[4].status = !activeAreas.value[4].status;
        break;
      case "legs":
        activeAreas.value[5].status = !activeAreas.value[5].status;
        break;
      case "hands":
        activeAreas.value[6].status = !activeAreas.value[6].status;
        break;
      default:
        return;
    }
  ...
  let disabled = $state(
    !activeAreas.value[0].status &&
    !activeAreas.value[1].status && 
    !activeAreas.value[2].status &&
    !activeAreas.value[3].status &&
    !activeAreas.value[4].status &&
    !activeAreas.value[5].status &&
    !activeAreas.value[6].status
);

  const handleSave = async () => {
    // Save the affected areas to the logged in User.
    // and switch onboarding off if it's our first time.
    await modifyUserAreas(usersettings.value, activeAreas.value);
    await downloadUser(user.value);
    await setupUserExercises(exercises.value, usersettings.value);
    landing.value = false;
    goto("/");
  };
</script>
...
<selector>
  <div class="human-body">
    ...
    >
      <path
        class={activeAreas.value[0].status ? "active" : ""}
    ...
    >
      <path
        class={activeAreas.value[1].status ? "active" : ""}
    ...
    >
      <path
        class={activeAreas.value[2].status ? "active" : ""}
    ...
    >
      <path
        class={activeAreas.value[3].status ? "active" : ""}
    ...
    >
      <path
        class={activeAreas.value[4].status ? "active" : ""}
    ...
    >
      <path
        class={activeAreas.value[5].status ? "active" : ""}
    ...
    >
      <path
        class={$activeAreas.value[6].status ? "active" : ""}
  ...
  <div class="legend">
    <Button variant="raised" disabled={disabled} on:click={handleSave}>SAVE AREAS</Button>
  </div>
</selector>
