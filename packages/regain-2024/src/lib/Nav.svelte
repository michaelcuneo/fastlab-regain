<script lang="ts">
	...
	import { user, usersettings, landing } from "$lib/store";
...
{#if !landing.value}
	...
	>
		<a aria-label="landing" href="/" on:click={handleLanding}>
			<svg
				width="163"
				height="40"
				viewBox="0 0 163 40"
		...
		<p>
			<Button color="secondary" on:click={() => goto("/help")}>
				<ButtonLabel>Help</ButtonLabel>
				<Icon class="material-icons">help</Icon>
			</Button>
			{#if !isEmpty($user)}
				{#if $usersettings.onboard === false}
					<Button color="secondary" on:click={() => goto("/")}>
						<ButtonLabel>Exercises</ButtonLabel>
						<Icon class="material-icons">home</Icon>
					</Button>
					<Button color="secondary" on:click={() => goto("/progress")}>
						<ButtonLabel>Progress</ButtonLabel>
						<Icon class="material-icons">forest</Icon>
					</Button>
					<Button color="secondary" on:click={() => goto("/settings")}>
